...
for i in range(1, emotions.nrows):
    ...
    if fila[1].value not in emotion_dic[fila[3].value]:
        emotion_dic[fila[3].value].append(fila[1].value)
...
try:
    ...
    for comment in tokenized_comments:
        ...
        for word in comment:
            ...
            for i in range(0, len(emotion_dic.keys())):
                mod_E = len(emotion_dic[list(emotion_dic.keys())[i]])
                emotion = list(emotion_dic.keys())[i]
                emotion_words = emotion_dic[emotion]
                word_ocurrence = words_ocu_dic[word]
                sum_ = 0
    ...
    result_file = open(f"exit_files/Result {argv[1].split('/')[1]}", 'w',
                       encoding="utf-8")
...
exit_files/Result {argv[1].split('/')[1]}")
except IndexError:
    print(dedent("""
