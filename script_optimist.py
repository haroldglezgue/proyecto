...
try:
    ...
    result_file = open(f"exit_files/Result_optimista {argv[1].split('/')[1]}",
                    'w', encoding="utf-8")
    result_json = open(f"exit_files/Result_optimista {argv[1].split('/')[1].split('.')[0]}.json",
                    'w', encoding="utf-8")
...
exit_files/Result_optimista {argv[1].split('/')[1]}")
except IndexError:
    print(dedent("""
